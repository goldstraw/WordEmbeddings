...
fn load_imdb_dataset(path: &str, num_reviews: usize) -> Vec<String> {
    let mut imdb_dataset = Vec::new();
    let mut reader = csv::Reader::from_path(path).unwrap();
    for result in reader.records() {
        let record = result.unwrap();
        let imdb_review = clean_review(record[0].to_string());
        imdb_dataset.push(imdb_review);
        if imdb_dataset.len() == num_reviews {
            break;
        }
    }
    imdb_dataset
...
fn build_co_occurrence_matrix(vocab: &Vec<String>, imdb_dataset: &Vec<String>) -> Vec<Vec<f32>> {
    ...
    for review in imdb_dataset {
        let words = review.split_whitespace();
...
pub fn run(dimensionality: usize, num_reviews: usize, output_name: &str, num_threads: usize) {
    let imdb_dataset = load_imdb_dataset("imdb_dataset.csv", num_reviews);
