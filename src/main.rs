...
fn main() {
    ...
    println!("Enter the number of reviews to process: ");
    let mut input = String::new();
    io::stdin().read_line(&mut input).expect("Failed to read input.");
    let num_reviews = input.trim().parse().expect("Invalid input.");
    ...
    run::run(dimensionality, num_reviews, file_name, num_threads);
}
